...
SoftwareWire::SoftwareWire(uint8_t sdaPin, uint8_t sclPin, boolean pullups, boolean detectClockStretch)
{
  ...
  _sdaPortReg  = (unsigned char*) portOutputRegister(port);
  _sdaDirReg   = (unsigned char*) portModeRegister(port);
  _sdaPinReg   = (unsigned char*) portInputRegister(port);      // PinReg is the input register, not the Arduino pin.

  port = digitalPinToPort(_sclPin);
  _sclBitMask  = digitalPinToBitMask(_sclPin);
  _sclPortReg  = (unsigned char*) portOutputRegister(port);
  _sclDirReg   = (unsigned char*) portModeRegister(port);
  _sclPinReg   = (unsigned char*) portInputRegister(port);
}
...
void SoftwareWire::printStatus( HardwareSerial& Ser)
{
  ...
  Ser.println( (size_t) _sdaPortReg, HEX);
  Ser.print(F("  _sclPortReg = "));  
  Ser.println( (size_t) _sclPortReg, HEX);
  Ser.print(F("  _sdaDirReg = "));  
  Ser.println( (size_t) _sdaDirReg, HEX);
  Ser.print(F("  _sclDirReg = "));  
  Ser.println( (size_t) _sclDirReg, HEX);
  Ser.print(F("  _sdaPinReg = "));  
  Ser.println( (size_t) _sdaPinReg, HEX);
  Ser.print(F("  _sclPinReg = "));  
  Ser.println( (size_t) _sclPinReg, HEX);
